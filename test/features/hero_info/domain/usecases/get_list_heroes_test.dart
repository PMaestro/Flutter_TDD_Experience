...
void main() {
  GetRandomHero usecase;
  MockHeroInfoRepository mockHeroInfoRepository;

  setUp(() {
    mockHeroInfoRepository = MockHeroInfoRepository();
    usecase = GetRandomHero(mockHeroInfoRepository);
  });
  ...
  test(
    ...
    () async {
      ...
      when(mockHeroInfoRepository.getRandomHero())
          .thenAnswer((_) async => Right(hero));
      final result = await usecase(NoParams());
      // The "act" phase of the test. Call the not-yet-existent method.
      // UseCase should simply return whatever was returned from the Repository
      expect(result, Right(hero));
      // Verify that the method has been called on the Repository
      verify(mockHeroInfoRepository.getRandomHero());
